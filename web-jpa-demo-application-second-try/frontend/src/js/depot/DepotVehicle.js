...
const DepotVehicles = () => {
    ...
    useEffect(() =>{
        if (!isNaN(parseInt(pageFromUrl, 10))) {
            ...
            history.replace(`/depot/vehicles?page=1`);
            setPage(1);
        }
    },[pageFromUrl])
    console.log(page);
    ...
    const [sortBy, setSortBy] = useState(localStorage.getItem('depotVehiclesSortBy' ) || 'fabricationYear');

    const [order, setOrder] = useState(localStorage.getItem('depotVehiclesOrder') || 'asc');

    const [searchBy, setSearchBy] = useState(localStorage.getItem('depotVehiclesSearchBy' ) || 'id');

    const [searchValue, setSearchValue] = useState(localStorage.getItem('depotVehiclesSearchValue' ) || '');
    ...
    const togglePreviousPage = () => {
        ...
        history.push(`/depot/vehicles?page=${page - 1}`)
    ...
    const handleSortChange = (event) => {
        ...
        setSortBy(value);
        localStorage.setItem('depotVehiclesSortBy', value);
    }
    ...
    const handleOrderChange = (event) => {
        const { value } = event.target;
        setOrder(value);
        localStorage.setItem('depotVehiclesOrder', value);
    }

    const handleSearchChange = (event) => {
        const { value } = event.target;
        setSearchBy(value);
        localStorage.setItem('depotVehiclesSearchBy', value);
    }

    const handleSearchValueChange = (event) => {
        const { value } = event.target;
        setSearchValue(value);
        localStorage.setItem('depotVehiclesSearchValue', value);
    }
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch(`http://localhost:2810/depot/vehicle`,{
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${localStorage.getItem('jwt')}`
                ...
                const responseDepot = await fetch(`http://localhost:2810/depot`,{
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `Bearer ${localStorage.getItem('jwt')}`
                ...
                const bodyDepot = await responseDepot.json();
                console.log('Response:', body);
                console.log('Response Depot:', bodyDepot);
                setClients(body);
                setDepot(bodyDepot);
    ...
    useEffect(() => {
        if ((page > totalPages && totalPages > 0) || page <= 0) {
            ...
            setPage(1);
            history.push(`/depot/vehicles?page=1`)
        }
    }, [page, totalPages]);
